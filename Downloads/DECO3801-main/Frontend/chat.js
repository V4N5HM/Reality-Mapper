...
async function fetchImageFromGoogle(label) {
  const apiKey = "AIzaSyAWOa1qQbiQ1Mm17w8wOC-LXCR30xRCCEw";
  const cx     = "f72582d4e03ac4628";
  const query  = encodeURIComponent(label);
  const url    = `https://www.googleapis.com/customsearch/v1?q=${query}&searchType=image&num=1&key=${apiKey}&cx=${cx}`;
